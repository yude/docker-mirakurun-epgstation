...
# Load EPGS-to-Discord-v2
ADD ./EPGS-to-Discord-v2 /app/EPGS-to-Discord-v2

ENV DEV="autoconf automake bash binutils bzip2 cmake coreutils diffutils file g++ gcc gperf libtool make python3 openssl-dev tar yasm nasm zlib-dev expat-dev pkgconfig libass-dev lame-dev opus-dev libtheora-dev libvorbis-dev libvpx-dev x264-dev x265-dev libva-dev"
...
RUN apk add --no-cache curl libgcc libstdc++ ca-certificates libcrypto1.1 libssl1.1 libgomp expat git lame libass libvpx opus libtheora libvorbis x264-libs x265-libs libva $DEV && \
# aribb24
